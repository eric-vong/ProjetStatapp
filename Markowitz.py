...
def eigvals_density(mean,cov,sample_size = 250,iteration=100,nombre_bins = 100):
    bins = np.linspace(0,2.5,nombre_bins+1)
...
def denoise_rmt_chosen(cov_estim,k): #k = threshold?
...
def comparison_rmt(mean,cov,sample_size = 250, lambdas = 100,k=20):
    ...
    R_lw,sigma_lw = R_sigma_computation(mean,inv_cov_estim_lw,cov,risk_aversion_list)
    return R,sigma,R_rmt,sigma_rmt,R_lw,sigma_lw

R_empi,sigma_empi,R_rmt,sigma_rmt,R_shrink,sigma_shrink = comparison_rmt(mean,cov,sample_size=500,lambdas = 100)
